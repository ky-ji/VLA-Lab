...
class StreamlitVisualizer:
    ...
    def plot_interactive_step(self, episode_idx: int, step_idx: int):
        ...
        with c2:
            ...
            if action_dim >= 8:
                ...
                g_state = "OPEN 👐" if g_val > 0.5 else "CLOSED ✊" # 假设归一化了，根据实际情况调整
        ...
        # 画当前点 (大红点)
        ax1.scatter(current_action[0], current_action[1], current_action[2], c='r', s=150, edgecolors='k', label='Current', zorder=100)
    ...
    def plot_workspace_3d(self, sample_ratio=0.1):
...
st.set_page_config(layout="wide", page_title="RoboData Visualizer")
st.sidebar.title("🤖 训练数据可视化")

# 1. 路径输入
default_path = "/home/jikangye/workspace/baselines/vla-baselines/RealWorld-DP/data/demo_test.zarr" 
zarr_path = st.sidebar.text_input("Zarr 数据集路径", value=default_path)
...
if st.sidebar.button("加载/重载数据集", type="primary"):
    if Path(zarr_path).exists() or Path(zarr_path).is_dir():
        st.session_state.viz = StreamlitVisualizer(zarr_path)
        st.success(f"已加载: {Path(zarr_path).name}")
    else:
        st.error("路径不存在！")
...
if viz and viz.valid:
    ...
    # 主要功能区
    tab1, tab2, tab3 = st.tabs(["🔍 详细交互 (Interactive)", "📊 全局概览 (Overview)", "🧊 空间分布"])
    ...
    with tab1:
        ...
        # 核心交互滑块
        step_idx = st.slider("⏱️ 时间轴 (拖动滑块查看对应图像和动作)", 0, max_step, 0, key="timeline_slider")
